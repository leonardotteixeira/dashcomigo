  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white">Metas de Receita</h1>
          <p className="text-[#A1A1A1] text-sm mt-1">Defina e acompanhe suas metas mensais</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="bg-[#1B1B1B] border border-white/5 rounded-2xl p-6 mb-6">
          <div className="flex items-start justify-between mb-4">
            <div>
              <p className="text-[#A1A1A1] text-sm">{formatMes(mesAtual())}</p>
              <h2 className="text-white text-2xl font-bold mt-1">
                {fmt(metaMesAtual.valorRealizado)}
                <span className="text-[#A1A1A1] text-base font-normal"> / {fmt(metaMesAtual.valorMeta)}</span>
              </h2>
              {metaMesAtual.categoria && (
                <p className="text-[#A1A1A1] text-xs mt-1">Foco: {metaMesAtual.categoria}</p>
              )}
            </div>
          ...
          <div className="flex justify-between text-xs text-[#A1A1A1]">
            ...
            <button onClick={() => handleDelete(metaMesAtual.id)} className="text-[#A1A1A1] hover:text-red-400 transition-colors flex items-center gap-1">
        ...
        <div className="bg-[#1B1B1B] border border-dashed border-white/10 rounded-2xl p-8 mb-6 text-center">
          <Target className="w-10 h-10 text-[#A1A1A1] mx-auto mb-3" />
          <p className="text-[#A1A1A1] mb-2">Nenhuma meta definida para {formatMes(mesAtual())}</p>
          <button
            onClick={() => { resetForm(); setModalOpen(true); }}
            className="text-[#2DDB81] text-sm hover:underline"
          >
            Definir meta do mês
          </button>
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {historico.map((goal) => {
              ...
              return (
                <div
                  key={goal.id}
                  className={`bg-[#1B1B1B] border rounded-2xl p-4 ${
                ...
                >
                  <p className="text-[#A1A1A1] text-xs">
                    ...
                    {isAtual && <span className="ml-1 text-[#2DDB81]">• atual</span>}
                  </p>
                  ...
                  <p className="text-[#A1A1A1] text-xs mt-1.5">
          ...
          <div className="bg-[#1B1B1B] border border-white/10 rounded-2xl w-full max-w-sm">
            <div className="p-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="text-[#A1A1A1] text-sm mb-1 block">Meta de Receita (R$) *</label>
                  <input
                    type="number"
                    step="0.01"
                    min="0.01"
                ...
                <div>
                  <label className="text-[#A1A1A1] text-sm mb-1 block">Categoria focada (opcional)</label>
                  <input
                    type="text"
                    value={formCategoria}
